...
def detect_dbr(xl_file, sheet_name: str):
    ...
    # Cari kolom Kode Barang (10 digit)
    ...
    for i in range(min(len(raw), 60)):
        for j in range(min(nc, 15)):
            ...
            if pd.isna(v):
                continue
            if re.match(r"^\d{10}$", clean_kode(v)):
    ...
    # Header scan (backward)
    col_map = {"kode": kode_col}
    for sr in range(first_row-1, max(first_row-6, -1), -1):
        if sr < 0:
            break
    ...
    if upl_dbr:
        try:
            ...
            if st.button("⚙️ PROSES", type="primary", disabled=not sel, use_container_width=True):
                ...
                bar.progress(1.0, text=f"Selesai! {len(pdfs)} ruangan.")
                st.rerun()
        except Exception as e:
            st.error(f"❌ Gagal: {e}")
...
if s3_lock:
    ...
    if not results:
        ...
        for tab, (sh, rr) in zip(tabs, results.items()):
            with tab:
                ca, cb, cc = st.columns(3)
                ca.metric("Jumlah Label", len(rr.items))
                cb.metric("Ada di Master", sum(1 for it in rr.items if it.in_master))
                not_in = sum(1 for it in rr.items if not it.in_master)
