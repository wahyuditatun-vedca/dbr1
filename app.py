...
def extract_labels_from_pdf(pdf_bytes, source_name, doc_id):
    ...
    for pi in range(doc.page_count):
        ...
        PAD_TOP = 14  # logo space above KEMENTERIAN text
        col_split = pw / 2

        for rt in row_tops:
            cell_y0 = max(0, rt - PAD_TOP)
            cell_y1 = min(ph, cell_y0 + row_h)
            for col in (0,1):
                ...
                cell_spans = [(bb[1],t) for t,bb in spans
                              if cx0<=(bb[0]+bb[2])/2<=cx1 and cell_y0<=(bb[1]+bb[3])/2<=cell_y1]
...
def build_output_pdf(ruangan, matched_labels, docs_by_id):
    """Crop labels from master PDF. Only matched labels are included."""
    if not matched_labels: return b""
    doc = fitz.open()
    PAGE_W, PAGE_H = 595, 842
    MARGIN_X, MARGIN_TOP, MARGIN_BOTTOM = 20, 48, 20
    GUTTER_X, GUTTER_Y = 6, 4

    # Calculate cell dimensions from first label
    s = matched_labels[0]
    src_w = s.bbox[2] - s.bbox[0]
    src_h = s.bbox[3] - s.bbox[1]
    cell_w = (PAGE_W - 2*MARGIN_X - GUTTER_X) / 2
    scale = cell_w / src_w
    cell_h = src_h * scale

    rows_per_page = max(1, int((PAGE_H-MARGIN_TOP-MARGIN_BOTTOM+GUTTER_Y)/(cell_h+GUTTER_Y)))
    ...
    for i, lbl in enumerate(matched_labels):
        ...
        if pos == 0:
            ...
            page.insert_text(fitz.Point(MARGIN_X, 20),
                             f"DAFTAR LABEL BARANG — {ruangan.upper()}",
                             fontsize=9, fontname="hebo")
            pg = (i//cells_per_page)+1
            total = math.ceil(len(matched_labels)/cells_per_page)
            page.insert_text(fitz.Point(PAGE_W-MARGIN_X-55, 20),
                             f"Hal {pg}/{total}", fontsize=7, fontname="helv")
            page.draw_line(fitz.Point(MARGIN_X,25), fitz.Point(PAGE_W-MARGIN_X,25),
        ...
        if src_doc is None: continue
        page.show_pdf_page(target, src_doc, lbl.page_index,
